...
function E_hfs(α_::atom_nos,α::atom_nos)
    α_==α || return 0u"hartree" # diagonal in atomic spin numbers
...
function H_hfs(bra::asym_αβlml_ket,ket::asym_αβlml_ket)
    (bra.l==ket.l && bra.ml==ket.ml) || return 0e0u"hartree" # diagonal in |l mₗ⟩
    return E_hfs(bra.α,ket.α) + E_hfs(bra.β,ket.β) # assumed independent of R
end
